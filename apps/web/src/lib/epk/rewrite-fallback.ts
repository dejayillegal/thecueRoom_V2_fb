...
function generateBlurb(parsed: ReturnType<typeof parseBio>, tone: Tone, seed: string): string {
...
function generateEPKBio(parsed: ReturnType<typeof parseBio>, tone: Tone, seed: string): string {
  const template = TONE_TEMPLATES[tone];
...
export function deterministicRewrite(text: string, tone: Tone = 'press'): RewriteResult {
  if (!text || text.trim().length === 0) {
    return {
      tagline: 'Electronic Music Artist',
      blurb: 'Innovative producer and performer bringing fresh energy to the electronic music scene.',
      epk_bio: 'An emerging artist in the electronic music landscape, dedicated to creating memorable experiences through sound. With a passion for innovation and a commitment to craft, they continue to evolve and inspire audiences worldwide.'
    };
  }
