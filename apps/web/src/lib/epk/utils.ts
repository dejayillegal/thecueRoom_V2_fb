...
export function debouncedLocalSave(key: string, data: any, delay: number = 500) {
  const save = debounce(() => {
    try {
...
export async function pollJobStatus(jobId: string, onProgress?: (progress: number) => void): Promise<any> {
...
export type EPKModuleType = 'bio' | 'tracklist' | 'gallery' | 'techRider' | 'links' | 'quotes';
