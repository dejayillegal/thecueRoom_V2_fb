...
export async function verifyUserProfile(
  ...
  if (
    ...
    !user.email.includes("disposable")
  ) {
    score += 10;
  }
...
async function analyzeFakeAccountIndicators(
  ...
  if (!profile.socialProfileUrl || profile.socialProfileUrl.length < 10) {
    ...
    try {
      ...
      if (
        suspiciousUrlPatterns.some((pattern) => url.hostname.includes(pattern))
      ) {
        suspicionScore += 30;
      ...
      if (url.pathname.length < 3) {
        suspicionScore += 10;
        indicators.push(
          "Social URL appears to be a homepage, not an artist profile",
        );
      }
    } catch (e) {
      suspicionScore += 25;
  ...
  if (profileHtml) {
    ...
    if (errorIndicators.some((indicator) => htmlLower.includes(indicator))) {
      suspicionScore += 35;
      indicators.push(
        "Social profile appears to be deleted, suspended, or not found",
      );
    }
    ...
    const hasMusicContent = musicIndicators.some((indicator) =>
  ...
  if (profile.artistName && user.username) {
    const normalizedArtist = profile.artistName
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "");
    ...
    if (normalizedArtist.length > 3 && normalizedUsername.length > 3) {
      const similarity =
        normalizedUsername.includes(normalizedArtist) ||
