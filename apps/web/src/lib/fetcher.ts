 ...
 */
export function safeParseJSON<T = any>(text: string): { ok: true; data: T } | { ok: false; text: string; error: string } {
  try {
    const data = JSON.parse(text) as T;
    return { ok: true, data };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : 'Invalid JSON';
    console.error('[safeParseJSON] Parse error:', errorMessage, 'Raw text:', text.substring(0, 200));
  ...
  for (let i = 0; i <= retries; i++) {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        throw new Error(`JSON Parse Error: ${result.error}. Response was: ${result.text.substring(0, 100)}`);
      }
