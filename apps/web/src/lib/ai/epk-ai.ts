...
export async function generateEPKText(
  request: GenerateEPKTextRequest,
): Promise<GenerateEPKTextResponse> {
  const prompt = buildPrompt(request);
  const systemMessage =
    "You are an expert music industry copywriter specializing in Electronic Press Kits (EPKs). Create compelling, professional content for artists.";
  ...
  console.warn(
    "[EPK AI] All AI providers failed or unavailable, using fallback",
  );
  return {
...
export async function improveEPKText(
  text: string,
  tone: string = "professional",
): Promise<GenerateEPKTextResponse> {
  const toneInstructions = {
    professional:
      "Make it more professional and polished for promoters and venues",
    ...
    press:
      "Rewrite for press releases - newsworthy, achievement-focused, media-friendly",
  };
  ...
  const systemMessage =
    "You are an expert music industry copywriter. Improve the following text while maintaining the core message.";
  ...
  if (openai && OPENAI_API_KEY) {
    try {
      ...
      console.error(
        "[EPK AI] OpenAI improvement error:",
        error?.message || error,
      );
    }
  }
  ...
  if (perplexity && PERPLEXITY_KEY) {
    try {
      ...
      console.error(
        "[EPK AI] Perplexity improvement error:",
        error?.message || error,
      );
    }
  }
  ...
  if (HUGGINGFACE_KEY) {
    try {
      ...
      console.error(
        "[EPK AI] Hugging Face improvement error:",
        error?.message || error,
      );
    }
  }
  ...
  if (GOOGLE_API_KEY) {
    try {
      ...
      console.error(
        "[EPK AI] Google Gemini improvement error:",
        error?.message || error,
      );
    }
  }
  ...
  console.warn(
    "[EPK AI] All AI providers failed for improvement, keeping original text",
  );
  return {
    text: text,
    usedAI: false,
