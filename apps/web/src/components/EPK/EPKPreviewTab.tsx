  ...
  const pollJobStatus = async (jobId: string, format: string) => {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        setExportStatus(
          `Generating ${format.toUpperCase()}... ${Math.round((attempts / maxAttempts) * 100)}%`,
        );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Review your EPK and export when ready
          </p>
        </div>
        {isLoadingPreview && (
      ...
      <div className="bg-[#0a0a0a] border border-[#222] rounded-lg p-4">
        <div
          className="bg-white border-4 border-[#333] rounded-lg overflow-hidden"
