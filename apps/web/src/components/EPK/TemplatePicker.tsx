...
export default function TemplatePicker({ onSelect, selectedTemplateId }: TemplatePickerProps) {
  ...
  useEffect(() => {
    async function loadTemplates() {
      try {
        ...
        const data = await safeParseJSON<{ ok: boolean; templates: Template[] }>(response);
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="dashboard-card h-48 animate-pulse bg-[#1a1a1a]" />
        ))}
      </div>
    );
  }
