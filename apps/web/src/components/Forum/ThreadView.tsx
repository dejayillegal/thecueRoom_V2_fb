...
export default function ThreadView({ threadId }: ThreadViewProps) {
  const { thread, replies, loading, error, addReply, likeThread } = useThread(threadId);
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {replies?.map((reply: any) => (
          <div key={reply.id} className="bg-gray-800 p-4 rounded-lg border border-gray-700">
            ...
            {reply.images?.length > 0 && (
              <div className="mt-3 grid grid-cols-2 gap-2">
                {reply.images.map((img: string, idx: number) => (
                  <img key={idx} src={img} alt="" className="rounded-lg max-h-48 object-cover" />
                ))}
              </div>
            )}
          </div>
