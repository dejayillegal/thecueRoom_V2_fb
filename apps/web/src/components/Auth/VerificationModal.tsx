...
export default function VerificationModal({
  ...
  const renderContent = () => {
    ...
    if (job.status === "failed") {
      return (
        <div
          className="p-6 bg-red-950/20 border border-red-500/50 rounded-lg"
          role="alert"
        >
          <div className="flex items-start gap-4">
            <XCircle
              className="h-8 w-8 text-red-500 flex-shrink-0 mt-1"
              aria-hidden="true"
            />
            <div>
              ...
              <p className="text-gray-300 mb-4">
                We encountered an error during verification. Please try again or
                contact support.
              </p>
              <Button
                onClick={onComplete}
    ...
    if (job.status === "completed" && job.decision === "verified") {
      return (
        <div
          className="p-6 bg-green-950/20 border border-green-500/50 rounded-lg"
          role="status"
          aria-live="polite"
        >
          <div className="flex items-start gap-4">
            <CheckCircle2
              className="h-12 w-12 text-green-500 flex-shrink-0 animate-pulse"
              aria-hidden="true"
            />
            <div className="flex-1">
              ...
              <p className="text-gray-300 mb-4">
                Congratulations! Your account has been verified. You now have
                full access to thecueRoom.
              </p>
              {job.confidence && (
                <div className="mb-4">
    ...
    if (job.status === "completed" && job.decision === "pending_admin") {
      return (
        <div
          className="p-6 bg-yellow-950/20 border border-yellow-500/50 rounded-lg"
          role="status"
          aria-live="polite"
        >
          <div className="flex items-start gap-4">
            <Clock
              className="h-8 w-8 text-yellow-500 flex-shrink-0 mt-1"
              aria-hidden="true"
            />
            <div>
              ...
              <p className="text-gray-300 mb-4">
                Your account is pending admin approval. We've notified our team
                and they'll review your profile shortly.
              </p>
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <Loader2
            className="h-12 w-12 animate-spin text-[#D7FF3C] mx-auto mb-4"
            aria-hidden="true"
          />
          <h3 className="text-xl font-semibold text-white mb-2">
            AI Profile Verification in Progress
          </h3>
          <p className="text-gray-400">
            Our AI is verifying your profile URL and checking for
            authenticity...
          </p>
        </div>
        ...
        <div className="space-y-3" aria-live="polite">
          {PROGRESS_STEPS.map((step, index) => {
            ...
            const isCurrent =
              progress < step.value &&
              (index === 0 || progress >= PROGRESS_STEPS[index - 1].value);
            ...
            return (
              <div key={step.key} className="flex items-center gap-3">
                {isComplete ? (
                  ...
                  <Loader2
                    className="h-5 w-5 text-[#D7FF3C] animate-spin"
                ...
                <span
                  className={`text-sm ${isComplete ? "text-green-400" : isCurrent ? "text-[#D7FF3C]" : "text-gray-500"}`}
                >
                  {step.label}
                </span>
              </div>
