...
const PROGRESS_STEPS = [
  ...
  { key: 'deciding', label: 'AI Decision', value: 80 },
  { key: 'complete', label: 'Complete', value: 100 }
];
...
export default function VerificationModal({ jobId, onComplete }: VerificationModalProps) {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (data.ok && data.job) {
          ...
          if (data.job.status === 'queued') setProgress(20);
            ...
            // Start auto-redirect countdown for verified users
  ...
  const renderContent = () => {
    ...
    if (job.status === 'failed') {
      return (
        <Card className="bg-red-950/20 border-red-500/50 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <Button onClick={onComplete} className="mt-4 bg-red-500 hover:bg-red-600 text-white">
                Close
              </Button>
            </div>
          </div>
    ...
    if (job.status === 'completed' && job.decision === 'verified') {
      return (
        <Card className="bg-green-950/20 border-green-500/50 p-6">
          <div className="flex items-start gap-4">
            <CheckCircle2 className="h-12 w-12 text-green-500 flex-shrink-0 animate-pulse" />
    ...
    if (job.status === 'completed' && job.decision === 'pending_admin') {
      return (
        <Card className="bg-yellow-950/20 border-yellow-500/50 p-6">
          <div className="flex items-start gap-4">
            <Clock className="h-8 w-8 text-yellow-500 flex-shrink-0 mt-1" />
            <div>
              ...
              {job.evidence && job.evidence.length > 0 && (
                <div className="mb-4">
                  <p className="text-sm font-semibold text-gray-300 mb-2">Submitted Information:</p>
                  <ul className="text-sm text-gray-400 space-y-1">
                    {job.evidence.map((item, i) => (
                      <li key={i}>• {item}</li>
                    ))}
                  </ul>
                </div>
              )}
              <Button onClick={onComplete} variant="outline" className="border-yellow-500/50 text-yellow-400 hover:bg-yellow-950/30">
                Close
              </Button>
            </div>
          </div>
      ...
      return (
        <Card className="bg-red-950/20 border-red-500/50 p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-xl font-semibold text-red-400 mb-2">Verification Not Successful</h3>
              <p className="text-gray-300 mb-4">
              ...
              <p className="text-sm text-gray-400 mb-4">
                Please update your social links with more professional profiles (SoundCloud, Spotify, etc.) and try again.
              </p>
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <Loader2 className="h-12 w-12 animate-spin text-[#D7FF3C] mx-auto mb-4" />
            ...
            return (
              <div key={step.key} className="flex items-center gap-3">
                {isComplete ? (
                  ...
                  <Loader2 className="h-5 w-5 text-[#D7FF3C] animate-spin" />
                ) : (
  ...
  return (
    <Dialog open={true} onOpenChange={canClose ? handleClose : undefined}>
      <DialogContent 
