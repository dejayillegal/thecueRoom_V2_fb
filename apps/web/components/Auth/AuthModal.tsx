...
import { AlertCircle, Loader2, X, Eye, EyeOff, ChevronRight } from 'lucide-react';
          ...
          <div className="px-8 pt-12 pb-6 sm:px-12 flex flex-col items-center text-center">
            <div className="flex flex-col items-center gap-4 mb-6">
              ...
              <div className="space-y-1">
                <h2 className="text-xl font-light tracking-[0.4em] uppercase text-white">thecueRoom</h2>
                <p className="text-[9px] font-mono tracking-[0.3em] uppercase text-zinc-500">Music, culture & creative intelligence</p>
              </div>
            </div>
            ...
            <div className="flex w-full bg-white/[0.02] border border-white/[0.05] p-1 rounded-none mt-4">
              {(['signin', 'signup', 'forgot'] as const).map((t) => (
                <button 
                  key={t}
                  onClick={() => setTab(t)}
                  className={`flex-1 py-3 text-[9px] uppercase tracking-[0.2em] font-bold transition-all duration-300 relative ${tab === t ? 'text-[#D1FF3D]' : 'text-zinc-600 hover:text-zinc-400'}`}
                >
                  <span className="relative z-10">{t === 'signin' ? 'Entrance' : t === 'signup' ? 'Registry' : 'Recovery'}</span>
                  {tab === t && (
                    <motion.div 
                      layoutId="activeRail" 
                        ...
                        <div className="space-y-6">
                          ...
                          >
                            {isSubmitting ? <Loader2 className="animate-spin w-4 h-4" /> : (
                              <>
                                {tab === 'signin' ? 'Enter' : 'Recover'} <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                              </>
                            )}
                          </Button>
